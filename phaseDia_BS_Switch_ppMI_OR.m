...
% Phase diagram calculatios of canonical and noncanonical bistable switches
...
% loading parameter set for the initial state (or switch) of the phase 
% diagram calculation. The mat file below contains the parameters obtained
% from random search. 
load parscan_ppMI_OR_R1.mat         
parM=ppMI_OR_dataR.para_iso;        % parameter for isola as initial state


% parM=ppMI_OR_dataR.para_invis and kk=3 (line: 469) for phase dia of inv-isola
% parM=ppMI_OR_dataR.para_invis and kk=11 (line: 469) for phase dia of inv-isola
% parM=ppMI_OR_dataR.para_mush and kk=4 (line: 469) for phase dia of mushroom
% parM=ppMI_OR_dataR.para_invmush and kk=4 (line: 469) for phase dia of inv-mushroom
...
% Range and step lengths of the two parameters against which phase diagram
% calculations are performed. 

gba0=10;                % initial value of gba
gab0=5;                 % initial value of gab
dp=5;                   % step length 
dim1=20;                % number of runs in gba direction
dim2=20;                % number of runs in gba direction

% each switch or phase has an uniue integer to represent it and it is
% stored in the phaseIndx matirx
phaseIndx=ones(dim1,dim2)*NaN;  
Val_gba=ones(dim1,dim2)*NaN;        % for storing of values of gba
Val_gab=ones(dim1,dim2)*NaN;        % for storing of values of gba

clr=flip(winter(dim2));

for kk=1:1           % loop for parameters
    kk=3             % choose kk to pick a particular parameter combination   
    ga0=parM(kk,1);
    gas=parM(kk,2);
    gab=parM(kk,3);
    gb0=parM(kk,4);
...
%             gab=90
            ...
            for jj=1:sigE1            % loop for bifurcation parameter
                ...
                f=gb0+gbs.*BS_int+gba.*BA_int-gmb.*b1;
            ...
            if max(nmbr_peaks_ss)==1
                phaseIndx(kp1,kp2)=0;       % monostable system    
            elseif max(nmbr_peaks_ss)==2
                kk;                         % that results bistability
                
                % to reduce computational cost bifurcation analysis is
                % carried out in the region of S that covers the two extreme
                % SN points (obtained from low-res bifurcation run) and some
                % extra regions on both sides
                kk0=find(nmbr_peaks_ss==2,1,'first');   % left most SN point
                kk1=find(nmbr_peaks_ss==2,1,'last');    % right most SN point
                dS=abs(kk0-kk1);      % dS: region between the two extreme SN points               
                if (dS <= 50)
                    sCut=15;
                elseif (50 < dS && dS <= 100)
                    sCut=25;
                ...
                % step length of bifurcation run is chosen based on dS
                if sum_diffss == 2
...
%                 sigE3=sigE3/dsig2;
                ...
                for j=1:sZ      % Loop for bifurcation parameter
                    ...
                    f2=gb0+gbs.*BS_int+gba.*BA_int-gmb.*b2;
                ...
                plot(vss(:,1),vss(:,2),'.','color','k','MarkerSize',2);
                hold on;
                plot(vss(:,1),vss(:,3),'.','color','k','MarkerSize',2);
                plot(vuss(:,1),vuss(:,2),'.','color','r','MarkerSize',2);
                set(gca,'XtickLabel',[])
                set(gca,'YtickLabel',[])
...
%                 bfrPt=find(diff(sign(nonNanUS)))+1;      % SN points from the unstable branch
                ...
                % segregation of switches based on jump patterns
...
% 
% ppMI_OR_dataR.total_bistability=[tot_bis tot_iso tot_bsF tot_bsB tot_inis ...
...
%  
% total_seg=sum(ppMI_OR_dataR.total_bistability)
% discrepency=total_seg-length(bistability_par)
% 
% ppMI_OR_dataR.data=parM;
% ppMI_OR_dataR.para_BS_all=parM;
% ppMI_OR_dataR.para_BS_pck=parM;
% 
% ppMI_OR_dataR.para_iso=para_iso;
% ppMI_OR_dataR.para_bsF=para_bsF;
% ppMI_OR_dataR.para_bsB=para_bsB;
% ppMI_OR_dataR.para_inis=para_inis;
% ppMI_OR_dataR.para_dlis=para_dlis;
% ppMI_OR_dataR.para_isbsBa=para_isbsBa;
% ppMI_OR_dataR.para_isbsFa=para_isbsFa;
% ppMI_OR_dataR.para_isbsBb=para_isbsBb;
% ppMI_OR_dataR.para_isbsFb=para_isbsFb;
% ppMI_OR_dataR.para_dlbsF=para_dlbsF;
% ppMI_OR_dataR.para_dlbsB=para_dlbsB;
% ppMI_OR_dataR.para_mus=para_mus;
% ppMI_OR_dataR.para_inmus=para_inmus;
% ppMI_OR_dataR.para_inisbsFb=para_inisbsFb;
% ppMI_OR_dataR.para_inisbsFa=para_inisbsFa;
% ppMI_OR_dataR.para_inisbsBb=para_inisbsBb;
% ppMI_OR_dataR.para_inisbsBa=para_inisbsBa;
% ppMI_OR_dataR.para_dlinis=para_dlinis;
% ppMI_OR_dataR.para_tlis=para_tlis;
% ppMI_OR_dataR.para_dlisbsFbb=para_dlisbsFbb;
% ppMI_OR_dataR.para_dlisbsFba=para_dlisbsFba;
% ppMI_OR_dataR.para_dlisbsFaa=para_dlisbsFaa;
% ppMI_OR_dataR.para_dlisbsBbb=para_dlisbsBbb;
% ppMI_OR_dataR.para_dlisbsBba=para_dlisbsBba;
% ppMI_OR_dataR.para_dlisbsBaa=para_dlisbsBaa;
% ppMI_OR_dataR.para_ismusb=para_ismusb;
% ppMI_OR_dataR.para_ismusm=para_ismusm;
% ppMI_OR_dataR.para_ismusa=para_ismusa;
% ppMI_OR_dataR.para_isdlbsFb=para_isdlbsFb;
% ppMI_OR_dataR.para_isdlbsFm=para_isdlbsFm;
% ppMI_OR_dataR.para_isdlbsFa=para_isdlbsFa;
% ppMI_OR_dataR.para_isdlbsBb=para_isdlbsBb;
% ppMI_OR_dataR.para_isdlbsBm=para_isdlbsBm;
% ppMI_OR_dataR.para_isdlbsBa=para_isdlbsBa;
% ppMI_OR_dataR.para_isinmusb=para_isinmusb;
% ppMI_OR_dataR.para_isinmusm=para_isinmusm;
% ppMI_OR_dataR.para_isinmusa=para_isinmusa;
% ppMI_OR_dataR.para_tpbsF=para_tpbsF;
% ppMI_OR_dataR.para_tpbsB=para_tpbsB;
% ppMI_OR_dataR.para_musbsFa=para_musbsFa;
% ppMI_OR_dataR.para_musbsBa=para_musbsBa;
% ppMI_OR_dataR.para_musbsFb=para_musbsFb;
% ppMI_OR_dataR.para_inmusbsBa=para_inmusbsBa;
% ppMI_OR_dataR.para_inmusbsFa=para_inmusbsFa;
% ppMI_OR_dataR.para_inmusbsBb=para_inmusbsBb;
% ppMI_OR_dataR.para_inisinmusb=para_inisinmusb;
% ppMI_OR_dataR.para_inisinmusm=para_inisinmusm;
% ppMI_OR_dataR.para_inisinmusa=para_inisinmusa;
% ppMI_OR_dataR.para_inismusb=para_inismusb;
% ppMI_OR_dataR.para_inismusm=para_inismusm;
% ppMI_OR_dataR.para_inismusa=para_inismusa;
% ppMI_OR_dataR.para_inisdlbsFb=para_inisdlbsFb;
% ppMI_OR_dataR.para_inisdlbsFm=para_inisdlbsFm;
% ppMI_OR_dataR.para_inisdlbsFa=para_inisdlbsFa;
% ppMI_OR_dataR.para_inisdlbsBb=para_inisdlbsBb;
% ppMI_OR_dataR.para_inisdlbsBm=para_inisdlbsBm;
% ppMI_OR_dataR.para_inisdlbsBa=para_inisdlbsBa;
% ppMI_OR_dataR.para_LbsB=para_LbsB;
% ppMI_OR_dataR.para_LbsF=para_LbsF;
% ppMI_OR_dataR.para_Ldlis=para_Ldlis;
% ppMI_OR_dataR.para_LbsFis=para_LbsFis;
% ppMI_OR_dataR.para_LbsBis=para_LbsBis;
% ppMI_OR_dataR.para_Linmus=para_Linmus;
% ppMI_OR_dataR.para_LdlbsF=para_LdlbsF;
% ppMI_OR_dataR.para_Lmus=para_Lmus;
% ppMI_OR_dataR.para_LbsBinis=para_LbsBinis;
% ppMI_OR_dataR.para_LbsFinis=para_LbsFinis;
% ppMI_OR_dataR.para_RbsF=para_RbsF;
% ppMI_OR_dataR.para_RbsB=para_RbsB;
% ppMI_OR_dataR.para_Rdlis=para_Rdlis;
% ppMI_OR_dataR.para_RbsBis=para_RbsBis;
% ppMI_OR_dataR.para_LbsFis=para_LbsFis;
% ppMI_OR_dataR.para_Rinmus=para_Rinmus;
% ppMI_OR_dataR.para_RbsFinis=para_RbsFinis;
% ppMI_OR_dataR.para_Rmus=para_Rmus;
% ppMI_OR_dataR.para_RbsBinis=para_RbsBinis;
% ppMI_OR_dataR.para_Rdlinis=para_Rdlinis;
% ppMI_OR_dataR.para_LdlisbsB=para_LdlisbsB;
% ppMI_OR_dataR.para_LdlisbsF=para_LdlisbsF;
% ppMI_OR_dataR.para_LdlbsBisa=para_LdlbsBisa;
% ppMI_OR_dataR.para_Linmusisa=para_Linmusisa;
% ppMI_OR_dataR.para_LdlbsBism=para_LdlbsBism;
% ppMI_OR_dataR.para_Linmusism=para_Linmusism;
% ppMI_OR_dataR.para_Lmusisa=para_Lmusisa;
% ppMI_OR_dataR.para_LdlbsFisa=para_LdlbsFisa;
% ppMI_OR_dataR.para_Lmusism=para_Lmusism;
% ppMI_OR_dataR.para_LdlbsFism=para_LdlbsFism;
% ppMI_OR_dataR.para_LbsBisinism=para_LbsBisinism;
% ppMI_OR_dataR.para_LtpbsB=para_LtpbsB;
% ppMI_OR_dataR.para_LbsBinmus=para_LbsBinmus;
% ppMI_OR_dataR.para_LmusbsB=para_LmusbsB;
% ppMI_OR_dataR.para_LinmusbsFa=para_LinmusbsFa;
% ppMI_OR_dataR.para_LbsBisinisa=para_LbsBisinisa;
% ppMI_OR_dataR.para_RdlisbsF=para_RdlisbsF;
% ppMI_OR_dataR.para_Rinmusism=para_Rinmusism;
% ppMI_OR_dataR.para_RdlbsFism=para_RdlbsFism;
% ppMI_OR_dataR.para_Rinmusisb=para_Rinmusisb;
% ppMI_OR_dataR.para_RdlbsFisb=para_RdlbsFisb;
% ppMI_OR_dataR.para_RdlisbsB=para_RdlisbsB;
% ppMI_OR_dataR.para_RinisisbsF=para_RinisisbsF;
% ppMI_OR_dataR.para_RinmusbsB=para_RinmusbsB;
% ppMI_OR_dataR.para_RinmusbsF=para_RinmusbsF;
% ppMI_OR_dataR.para_RdlbsBism=para_RdlbsBism;
% ppMI_OR_dataR.para_RmusbsF=para_RmusbsF;
% ppMI_OR_dataR.para_RtpbsF=para_RtpbsF;
% ppMI_OR_dataR.para_Rmusism=para_Rmusism;
% ppMI_OR_dataR.para_RisinisbsF=para_RisinisbsF;
% ppMI_OR_dataR.para_RdlbsBisb=para_RdlbsBisb;
% ppMI_OR_dataR.para_Rmusisb=para_Rmusisb;
% ppMI_OR_dataR.para_LRbs=para_LRbs;
% ppMI_OR_dataR.para_LRisis=para_LRisis;
% ppMI_OR_dataR.para_LRinisis=para_LRinisis;
% ppMI_OR_dataR.para_LRisinis=para_LRisinis;
% ppMI_OR_dataR.para_LRinisinis=para_LRinisinis;
% ppMI_OR_dataR.para_LRisisis=para_LRisisis;
% ppMI_OR_dataR.para_LRinisisis=para_LRinisisis;
% ppMI_OR_dataR.para_LRisbsBis=para_LRisbsBis;
% ppMI_OR_dataR.para_LRisbsFis=para_LRisbsFis;
% ppMI_OR_dataR.para_LRisisinis=para_LRisisinis;
% ppMI_OR_dataR.para_LRinisbsBis=para_LRinisbsBis;
% ppMI_OR_dataR.para_LRinisbsFis=para_LRinisbsFis;
% ppMI_OR_dataR.para_LRinisisinis=para_LRinisisinis;
% ppMI_OR_dataR.para_LRisinisis=para_LRisinisis;
% ppMI_OR_dataR.para_LRisbsBinis=para_LRisbsBinis;
% ppMI_OR_dataR.para_LRisbsFinis=para_LRisbsFinis;
% ppMI_OR_dataR.para_LRinisinisis=para_LRinisinisis;
% ppMI_OR_dataR.para_LRinisbsBinis=para_LRinisbsBinis;
% ppMI_OR_dataR.para_LRinisbsFinis=para_LRinisbsFinis;
% ppMI_OR_dataR.para_LRisbsFinis=para_LRisbsFinis;
% ppMI_OR_dataR.para_LRinisinisinis=para_LRinisinisinis;
% 
% ppMI_OR_dataR.indx_iso=iso_par;
% ppMI_OR_dataR.indx_bsF=bsF_par;
% ppMI_OR_dataR.indx_bsB=bsB_par;
% ppMI_OR_dataR.indx_inis=inis_par;
% ppMI_OR_dataR.indx_dlis=dlis_par;
% ppMI_OR_dataR.indx_isbsBa=isbsBa_par;
% ppMI_OR_dataR.indx_isbsFa=isbsFa_par;
% ppMI_OR_dataR.indx_isbsBb=isbsBb_par;
% ppMI_OR_dataR.indx_isbsFb=isbsFb_par;
% ppMI_OR_dataR.indx_dlbsF=dlbsF_par;
% ppMI_OR_dataR.indx_dlbsB=dlbsB_par;
% ppMI_OR_dataR.indx_mus=mus_par;
% ppMI_OR_dataR.indx_inmus=inmus_par;
% ppMI_OR_dataR.indx_inisbsFb=inisbsFb_par;
% ppMI_OR_dataR.indx_inisbsFa=inisbsFa_par;
% ppMI_OR_dataR.indx_inisbsBb=inisbsBb_par;
% ppMI_OR_dataR.indx_inisbsBa=inisbsBa_par;
% ppMI_OR_dataR.indx_dlinis=dlinis_par;
% ppMI_OR_dataR.indx_tlis=tlis_par;
% ppMI_OR_dataR.indx_dlisbsFbb=dlisbsFbb_par;
% ppMI_OR_dataR.indx_dlisbsFba=dlisbsFba_par;
% ppMI_OR_dataR.indx_dlisbsFaa=dlisbsFaa_par;
% ppMI_OR_dataR.indx_dlisbsBbb=dlisbsBbb_par;
% ppMI_OR_dataR.indx_dlisbsBba=dlisbsBba_par;
% ppMI_OR_dataR.indx_dlisbsBaa=dlisbsBaa_par;
% ppMI_OR_dataR.indx_ismusb=ismusb_par;
% ppMI_OR_dataR.indx_ismusm=ismusm_par;
% ppMI_OR_dataR.indx_ismusa=ismusa_par;
% ppMI_OR_dataR.indx_isdlbsFb=isdlbsFb_par;
% ppMI_OR_dataR.indx_isdlbsFm=isdlbsFm_par;
% ppMI_OR_dataR.indx_isdlbsFa=isdlbsFa_par;
% ppMI_OR_dataR.indx_isdlbsBb=isdlbsBb_par;
% ppMI_OR_dataR.indx_isdlbsBm=isdlbsBm_par;
% ppMI_OR_dataR.indx_isdlbsBa=isdlbsBa_par;
% ppMI_OR_dataR.indx_isinmusb=isinmusb_par;
% ppMI_OR_dataR.indx_isinmusm=isinmusm_par;
% ppMI_OR_dataR.indx_isinmusa=isinmusa_par;
% ppMI_OR_dataR.indx_tpbsF=tpbsF_par;
% ppMI_OR_dataR.indx_tpbsB=tpbsB_par;
% ppMI_OR_dataR.indx_musbsFa=musbsFa_par;
% ppMI_OR_dataR.indx_musbsBa=musbsBa_par;
% ppMI_OR_dataR.indx_musbsFb=musbsFb_par;
% ppMI_OR_dataR.indx_inmusbsBa=inmusbsBa_par;
% ppMI_OR_dataR.indx_inmusbsFa=inmusbsFa_par;
% ppMI_OR_dataR.indx_inmusbsBb=inmusbsBb_par;
% ppMI_OR_dataR.indx_inisinmusb=inisinmusb_par;
% ppMI_OR_dataR.indx_inisinmusm=inisinmusm_par;
% ppMI_OR_dataR.indx_inisinmusa=inisinmusa_par;
% ppMI_OR_dataR.indx_inismusb=inismusb_par;
% ppMI_OR_dataR.indx_inismusm=inismusm_par;
% ppMI_OR_dataR.indx_inismusa=inismusa_par;
% ppMI_OR_dataR.indx_inisdlbsFb=inisdlbsFb_par;
% ppMI_OR_dataR.indx_inisdlbsFm=inisdlbsFm_par;
% ppMI_OR_dataR.indx_inisdlbsFa=inisdlbsFa_par;
% ppMI_OR_dataR.indx_inisdlbsBb=inisdlbsBb_par;
% ppMI_OR_dataR.indx_inisdlbsBm=inisdlbsBm_par;
% ppMI_OR_dataR.indx_inisdlbsBa=inisdlbsBa_par;
% ppMI_OR_dataR.indx_LbsB=LbsB_par;
% ppMI_OR_dataR.indx_LbsF=LbsF_par;
% ppMI_OR_dataR.indx_Ldlis=Ldlis_par;
% ppMI_OR_dataR.indx_LbsFis=LbsFis_par;
% ppMI_OR_dataR.indx_LbsBis=LbsBis_par;
% ppMI_OR_dataR.indx_Linmus=Linmus_par;
% ppMI_OR_dataR.indx_LdlbsF=LdlbsF_par;
% ppMI_OR_dataR.indx_Lmus=Lmus_par;
% ppMI_OR_dataR.indx_LbsBinis=LbsBinis_par;
% ppMI_OR_dataR.indx_LbsFinis=LbsFinis_par;
% ppMI_OR_dataR.indx_RbsF=RbsF_par;
% ppMI_OR_dataR.indx_RbsB=RbsB_par;
% ppMI_OR_dataR.indx_Rdlis=Rdlis_par;
% ppMI_OR_dataR.indx_RbsBis=RbsBis_par;
% ppMI_OR_dataR.indx_LbsFis=LbsFis_par;
% ppMI_OR_dataR.indx_Rinmus=Rinmus_par;
% ppMI_OR_dataR.indx_RbsFinis=RbsFinis_par;
% ppMI_OR_dataR.indx_Rmus=Rmus_par;
% ppMI_OR_dataR.indx_RbsBinis=RbsBinis_par;
% ppMI_OR_dataR.indx_Rdlinis=Rdlinis_par;
% ppMI_OR_dataR.indx_LdlisbsB=LdlisbsB_par;
% ppMI_OR_dataR.indx_LdlisbsF=LdlisbsF_par;
% ppMI_OR_dataR.indx_LdlbsBisa=LdlbsBisa_par;
% ppMI_OR_dataR.indx_Linmusisa=Linmusisa_par;
% ppMI_OR_dataR.indx_LdlbsBism=LdlbsBism_par;
% ppMI_OR_dataR.indx_Linmusism=Linmusism_par;
% ppMI_OR_dataR.indx_Lmusisa=Lmusisa_par;
% ppMI_OR_dataR.indx_LdlbsFisa=LdlbsFisa_par;
% ppMI_OR_dataR.indx_Lmusism=Lmusism_par;
% ppMI_OR_dataR.indx_LdlbsFism=LdlbsFism_par;
% ppMI_OR_dataR.indx_LbsBisinism=LbsBisinism_par;
% ppMI_OR_dataR.indx_LtpbsB=LtpbsB_par;
% ppMI_OR_dataR.indx_LbsBinmus=LbsBinmus_par;
% ppMI_OR_dataR.indx_LmusbsB=LmusbsB_par;
% ppMI_OR_dataR.indx_LinmusbsFa=LinmusbsFa_par;
% ppMI_OR_dataR.indx_LbsBisinisa=LbsBisinisa_par;
% ppMI_OR_dataR.indx_RdlisbsF=RdlisbsF_par;
% ppMI_OR_dataR.indx_Rinmusism=Rinmusism_par;
% ppMI_OR_dataR.indx_RdlbsFism=RdlbsFism_par;
% ppMI_OR_dataR.indx_Rinmusisb=Rinmusisb_par;
% ppMI_OR_dataR.indx_RdlbsFisb=RdlbsFisb_par;
% ppMI_OR_dataR.indx_RdlisbsB=RdlisbsB_par;
% ppMI_OR_dataR.indx_RinisisbsF=RinisisbsF_par;
% ppMI_OR_dataR.indx_RinmusbsB=RinmusbsB_par;
% ppMI_OR_dataR.indx_RinmusbsF=RinmusbsF_par;
% ppMI_OR_dataR.indx_RdlbsBism=RdlbsBism_par;
% ppMI_OR_dataR.indx_RmusbsF=RmusbsF_par;
% ppMI_OR_dataR.indx_RtpbsF=RtpbsF_par;
% ppMI_OR_dataR.indx_Rmusism=Rmusism_par;
% ppMI_OR_dataR.indx_RisinisbsF=RisinisbsF_par;
% ppMI_OR_dataR.indx_RdlbsBisb=RdlbsBisb_par;
% ppMI_OR_dataR.indx_Rmusisb=Rmusisb_par;
% ppMI_OR_dataR.indx_LRbs=LRbs_par;
% ppMI_OR_dataR.indx_LRisis=LRisis_par;
% ppMI_OR_dataR.indx_LRinisis=LRinisis_par;
% ppMI_OR_dataR.indx_LRisinis=LRisinis_par;
% ppMI_OR_dataR.indx_LRinisinis=LRinisinis_par;
% ppMI_OR_dataR.indx_LRisisis=LRisisis_par;
% ppMI_OR_dataR.indx_LRinisisis=LRinisisis_par;
% ppMI_OR_dataR.indx_LRisbsBis=LRisbsBis_par;
% ppMI_OR_dataR.indx_LRisbsFis=LRisbsFis_par;
% ppMI_OR_dataR.indx_LRisisinis=LRisisinis_par;
% ppMI_OR_dataR.indx_LRinisbsBis=LRinisbsBis_par;
% ppMI_OR_dataR.indx_LRinisbsFis=LRinisbsFis_par;
% ppMI_OR_dataR.indx_LRinisisinis=LRinisisinis_par;
% ppMI_OR_dataR.indx_LRisinisis=LRisinisis_par;
% ppMI_OR_dataR.indx_LRisbsBinis=LRisbsBinis_par;
% ppMI_OR_dataR.indx_LRisbsFinis=LRisbsFinis_par;
% ppMI_OR_dataR.indx_LRinisinisis=LRinisinisis_par;
% ppMI_OR_dataR.indx_LRinisbsBinis=LRinisbsBinis_par;
% ppMI_OR_dataR.indx_LRinisbsFinis=LRinisbsFinis_par;
% ppMI_OR_dataR.indx_LRisbsFinis=LRisbsFinis_par;
% ppMI_OR_dataR.indx_LRinisinisinis=LRinisinisinis_par;
% 
% 
% save ppMI_OR_dataRRN_R1.mat ppMI_OR_dataR

save phaseDia_ppMI_Iso_r1p3_OR_r1.mat phaseIndx Val_gab Val_gba
savefig(fig1,'phaseDia_ppMI_Iso_r1p3_OR_r1')
saveas(fig1,'phaseDia_ppMI_Iso_r1p3_OR_r1','png')
% 
toc
% 
% quit
