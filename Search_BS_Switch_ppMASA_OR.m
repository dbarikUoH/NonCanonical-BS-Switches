...
load jas_in_ppMASA_OR.dat
load jab_in_ppMASA_OR.dat
load jba_in_ppMASA_OR.dat

% Sample size of parameter combinations
nr=50000;                    

size_jas=length(jas_in_ppMASA_OR);
size_jab=length(jab_in_ppMASA_OR);
size_jba=length(jba_in_ppMASA_OR);
...
Jas=jas_in_ppMASA_OR(rnd_as(1:nr));
Jbs=jas_in_ppMASA_OR(rnd_bs(1:nr));
Jab=jab_in_ppMASA_OR(rnd_ab(1:nr));
Jba=jba_in_ppMASA_OR(rnd_ba(1:nr));
Jbb=jba_in_ppMASA_OR(rnd_bb(1:nr));
...
sigE2=2000;         % For high-res run: no. of points in the S scan
dsig1=2.0;          % For low-res run: dS (interval of S)   
dsig2=0.5;         % For high-res run: dS (interval of S)
...
for kk=1:nr         % loop for parameters
    ...
    for jj=1:sigE1            % loop for bifurcation parameter
        ...
        BA_int=BA1./(1+BA1);
        BB1=(b1./Jbb(kk)).^nbb(kk);
        BB_int=BB1./(1+BB1);
        
        % force term in the euqation of B
        f = gb0(kk)+pb.*gbs(kk).*BS_int+gba(kk).*BA_int+gbb(kk).*BB_int-gmb(kk).*b1;
...
%             j1=sigE3F1+(j-1);
            ...
            BA1=(Ass./Jba(kk)).^nba(kk);
            BA_int=BA1./(1+BA1);
            BB1=(b2./Jbb(kk)).^nbb(kk);
            BB_int=BB1./(1+BB1);
        
            f2 = gb0(kk)+pb.*gbs(kk).*BS_int+gba(kk).*BA_int+gbb(kk).*BB_int-gmb(kk).*b1;
...
ppMASA_OR_dataR.total_bistability=[tot_bis tot_iso tot_inviso tot_dni tot_nibi ...
    tot_dbs tot_mush tot_invmush tot_iibi tot_nim tot_mbs tot_iim tot_nidbs ...
      tot_tbs tot_iidbs];
total_seg=sum(ppMASA_OR_dataR.total_bistability)
...
ppMASASA_OR_dataR.data=data;
ppMASA_OR_dataR.para_BS_all=para_BS_all;
ppMASA_OR_dataR.para_BS_pck=para_BS_pck;
ppMASA_OR_dataR.para_bis=para_bis;
ppMASA_OR_dataR.para_iso=para_iso;
ppMASA_OR_dataR.para_invis=para_inviso;
ppMASA_OR_dataR.para_dni=para_dni;
ppMASA_OR_dataR.para_nibi=para_nibi;
ppMASA_OR_dataR.para_dbs=para_dbs;
ppMASA_OR_dataR.para_mush=para_mush;
ppMASA_OR_dataR.para_invmush=para_invmush;
ppMASA_OR_dataR.para_iibi=para_iibi;
ppMASA_OR_dataR.para_nim=para_nim;
ppMASA_OR_dataR.para_mbs=para_mbs;
ppMASA_OR_dataR.para_iim=para_iim;
ppMASA_OR_dataR.para_nidbs=para_nidbs;
ppMASA_OR_dataR.para_tbs=para_tbs;
ppMASA_OR_dataR.para_iidbs=para_iidbs;

% storing of index of seggregated parameters
ppMASA_OR_dataR.indx_bis=bispar;
ppMASA_OR_dataR.indx_iso=isopar;
ppMASA_OR_dataR.indx_invis=invisopar;
ppMASA_OR_dataR.indx_dni=dnipar;
ppMASA_OR_dataR.indx_nibi=nibipar;
ppMASA_OR_dataR.indx_dbs=dbspar;
ppMASA_OR_dataR.indx_mush=mushpar;
ppMASA_OR_dataR.indx_invmush=invmushpar;
ppMASA_OR_dataR.indx_iibi=iibipar;
ppMASA_OR_dataR.indx_nim=nimpar;
ppMASA_OR_dataR.indx_mbs=mbspar;
ppMASA_OR_dataR.indx_iim=iimpar;
ppMASA_OR_dataR.indx_nidbs=nidbspar;
ppMASA_OR_dataR.indx_tbs=tbspar;
ppMASA_OR_dataR.indx_iidbs=iidbspar;

save parscan_ppMASA_OR_R1.mat ppMASA_OR_dataR
